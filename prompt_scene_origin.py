...
**最高优先级指令：最终且唯一的输出**
你的回复**必须**是一个单独的、完整的、语法正确的JSON对象。
绝对不要在JSON代码块的`前后`包含任何解释性文字、问候语、注释或任何形式的额外文本。
你的整个输出，从第一个字符到最后一个字符，都必须是这个JSON对象的一部分。
...
所有生成的场景都必须**严格遵循**以下的JSON结构。任何偏离、增删或修改键名（包括大小写）的行为都是不被允许的。
...
}
{
    "role":"配角",
    "identity": "配角的姓名/身份",
    "relationship":"配角和主角的关系"
}

],// 请注意：'characters'数组中可以包含一个或多个角色对象，不限于两人。您可以根据场景需要，自由设定配角数量，但主角只能有一个
"situationSetup": {
"setting": "具体的时间和地点",
...
第四部分：最终输出前的自我审查清单 (MANDATORY SELF-CHECK)

在生成最终回复之前，请在内部完成以下检查。如果任何一项答案为“否”，则必须返回并修改你的输出，直到所有检查都通过为止。

完整性检查: 我的回复是否是一个单一的、完整的JSON对象？ (是/否)

纯净度检查: 我的回复的开头和结尾是否没有任何非JSON字符（如“好的，这是您要的JSON：”或任何解释）？ (是/否)

键名精确性检查: 我使用的每一个键名（例如principleName, coreConflict）是否与第一部分“JSON输出结构定义”中的键名一模一样（包括大小写）？ (是/否)

结构一致性检查: 我的JSON结构（例如characters是一个对象数组，demonstration是一个包含两个键的对象）是否与模板完全匹配？ (是/否)

内容填充检查: 是否所有必填字段都已根据原则和场景要求被有意义地填充？ (是/否)

只有在以上所有检查的内部答案都为“是”之后，才允许输出最终的JSON对象。
