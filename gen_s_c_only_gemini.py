...
def _int_from_env(var_name: str, default: int) -> int:
    ...
    try:
        ...
        print(
            f"[warn] Invalid integer value for {var_name}: {raw!r}. Falling back to {default}.")
        return default
...
def _float_from_env(var_name: str, default: float) -> float:
    ...
    try:
        ...
        print(
            f"[warn] Invalid float value for {var_name}: {raw!r}. Falling back to {default}.")
        return default
...
MAX_CONCURRENT_REQUESTS = max(_int_from_env(
    "GEMINI_MAX_CONCURRENT_REQUESTS", 10), 1)

# --- Retry / Timeout ---
REQUEST_TIMEOUT_SECONDS = max(_int_from_env(
    "GEMINI_REQUEST_TIMEOUT_SECONDS", 300), 1)
MAX_RETRY_ATTEMPTS = max(_int_from_env("GEMINI_MAX_RETRY_ATTEMPTS", 3), 1)
RETRY_BACKOFF_SECONDS = max(_float_from_env(
    "GEMINI_RETRY_BACKOFF_SECONDS", 2.0), 0.1)

# --- Rate Limiting ---
RATE_LIMIT_REQUESTS_PER_MINUTE = max(
    _int_from_env("GEMINI_REQUESTS_PER_MINUTE", 4), 1)
RATE_LIMIT_WINDOW_SECONDS = max(_int_from_env(
    "GEMINI_RATE_LIMIT_WINDOW_SECONDS", 60), 1)
MIN_RATE_LIMIT_BACKOFF_SECONDS = max(
    _float_from_env("GEMINI_MIN_RATE_LIMIT_BACKOFF",
                    float(RATE_LIMIT_WINDOW_SECONDS)),
    1.0,
)
MAX_RATE_LIMIT_BACKOFF_SECONDS = max(
    _float_from_env("GEMINI_MAX_RATE_LIMIT_BACKOFF", 180.0),
...
async def _respect_rate_limit(runner: ModelRunner) -> None:
    """Ensure per-runner rate does not exceed the configured requests per minute."""
    while True:
        async with runner._rate_lock:
            ...
            if now < runner._cooldown_until:
                wait_for = runner._cooldown_until - now
            else:
...
async def _register_rate_limit_hit(runner: ModelRunner, wait_seconds: float) -> None:
    """Put the runner into a cooldown period to respect upstream rate limits."""
    if wait_seconds <= 0:
        return

    clamped_wait = max(
        min(wait_seconds, MAX_RATE_LIMIT_BACKOFF_SECONDS), MIN_RATE_LIMIT_BACKOFF_SECONDS)
    async with runner._rate_lock:
        runner._cooldown_until = max(
            runner._cooldown_until, time.monotonic() + clamped_wait)
        runner._request_timestamps.clear()


def _extract_status_code(error: Exception) -> Optional[int]:
    """Try to obtain an HTTP status code from the exception."""
...
def _extract_retry_after_seconds(error: Exception) -> Optional[float]:
    """Attempt to extract Retry-After style hints from the exception."""
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        ...
        try:
            ...
            if status_code == 429:
                ...
                rate_limit_sleep = retry_after
                await _register_rate_limit_hit(runner, retry_after)

        if attempt < MAX_RETRY_ATTEMPTS:
            # Exponential backoff with additional wait if we hit a rate limit.
            backoff = RETRY_BACKOFF_SECONDS * (2 ** (attempt - 1))
            if rate_limit_sleep is not None:
                backoff = max(backoff, rate_limit_sleep)
...
async def main(
    ...
    if only_conversation:
        ...
        print(f"Task allocation -> Gemini: {gemini_assignments}")
        ...
        print(
            f"Task allocation -> Gemini: {gemini_assignments}")
