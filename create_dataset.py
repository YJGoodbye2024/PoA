...
from prompt_scene import single_principle_sys_prompt,multi_principles_sys_prompt,single_principle_role_prompt,multi_principles_role_prompt
...
def generate_scenes_domain_1(domain):
    ...
    for i,item in enumerate(principles):
        if i > 0:
            break
        exclusion_list = find_similar_scenarios(item, top_k=3)
        formatted_role_prompt =single_principle_role_prompt.format(
            domain=domain,
            retrieved_summary_1=exclusion_list[0] if len(exclusion_list) > 0 else "无",
            retrieved_summary_2=exclusion_list[1] if len(exclusion_list) > 1 else "无",
            retrieved_summary_3=exclusion_list[2] if len(exclusion_list) > 2 else "无",
            principle=item
            )
        
        print(f"输入为："+single_principle_sys_prompt+formatted_role_prompt+"\n")
        ...
        if result1 is None:
            ...
            add_scenario_to_memory(result1)
            scenes.append(result1)
            print(f"{domain}领域单原则第{i+1}个原则生成scene成功: {result1}\n")
...
def generate_scenes_domain_2(domain):
    ...
    for i,item in enumerate(principles):
        ...
        for vlaue in pri_rel[item['principle']]:
            ...
            print(f"输入为："+multi_principles_sys_prompt+formatted_role_prompt+"\n")
            ...
            result1= extract_dict(response1.choices[0].message.content)
            if result1 is None:
                print(f"{domain}领域多原则第{i+1}个原则生成scene失败: {item['principle']}\n")
                continue
            else:
                add_scenario_to_memory(result1)
                scenes.append(result1)
                print(f"{domain}领域多原则第{i+1}个原则生成scene成功: {result1}\n")
