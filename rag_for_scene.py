...
def wait_for_vector_availability(scenario_id, max_retries=10, wait_time=2):
    """等待向量在Pinecone中变为可查询状态"""
    for attempt in range(max_retries):
        try:
            # 尝试通过ID获取刚插入的向量
            fetch_result = pinecone_index.fetch(ids=[scenario_id])
            if scenario_id in fetch_result.vectors:
                print(f"✅ 向量 {scenario_id} 已可查询")
                return True
        except:
            pass

        print(f"⏳ 等待向量索引 (尝试 {attempt + 1}/{max_retries})")
        time.sleep(wait_time)

    print(f"⚠️  向量 {scenario_id} 可能仍未完全索引")
    return False


def add_scenario_to_memory(generated_scenario_json, domain):
    ...
    metadata = {
        ...
        "all_principles": generated_scenario_json["principleName"],
        "scenario_title": generated_scenario_json["scenarioTitle"],
        "domain": domain,
        "creation_timestamp": int(time.time())
    }
    print(f"生成的metadata内容如下：{metadata}\n")
    # 4. 生成唯一ID并存入Pinecone
    ...
    try:
        ...
        # 等待向量变为可查询
        wait_for_vector_availability(scenario_id)

    except Exception as e:
        print(f"❌ 存入Pinecone失败: {e}")


def find_similar_scenarios(principle_json, top_k=3, domain='学习与成长'):
    """根据原则描述，在Pinecone中检索最相似的场景摘要"""

    # 1. 确定查询内容
    principle_description = principle_json["description"]
    principle_name = principle_json["principle"]
    print(f"查询的principle_name为{principle_name}")
    ...
    # 3. 执行向量搜索，并使用元数据过滤
    print(
        f"正在Pinecone中检索与'{domain}'场景下'{principle_name}'原则最相似的 {top_k} 个场景...")
    print(
        f"查询过滤条件为：1.【principle_name】:{principle_name}.    2. 【domain】:{domain}")
    try:
        search_results = pinecone_index.query(
            vector=query_vector,
            top_k=top_k,
            filter={
                # 关键过滤：只在同样领域、同样原则的场景中寻找相似项
                "principle_name": {"$eq": principle_name},
                "domain": {"$eq": domain}
            },
        ...
        # 4. 提取排斥摘要
        exclusion_summaries = [match['metadata']['summary']
